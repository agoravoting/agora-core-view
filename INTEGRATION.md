...
### 2. El usuario pincha en un botón dentro de la aplicación para votar.

Dicho botón sólo deberá aparecer en la aplicación en el caso de que haya una
votación en curso. Este punto es importante, porque estamos asumiendo que dicho
botón desaparecerá al terminar la votación, y por tanto la cabina de votación
no entrará en detalles de comprobar a priori si se está en periodo de votación
más que en el último paso, cuando se envía el voto al servidor.
...
referimos a diferentes preguntas. Nos referimos a dos procesos de votación
diferentes, con periodo de inicio y fin de votación diferentes, se solapen en
tiempos.
...
### 3. La aplicación de forma segura y autenticada solicita a los servidores de podemos unos credenciales para votar.
...
credencial que se incluirá en la url que en el paso siguiente (paso 6), y dicho
credencial:
...
razones de seguridad. De esa forma, el enlace que se genera en el paso 6 no
será valido pasado un corto periodo de tiempo, y será necesario generar desde
la aplicación un nuevo enlace si se quiere volver a votar.
...
y cuando lo obtenga lance un navegador web.

Cuando decimos que "la aplicación de forma segura y autenticada solicita a los
servidores de podemos unos credenciales", con "de forma segura" nos referimos a
que por ejemplo entendemos que la comunicación se hará por ejemplo a través de
un canal seguro, como por ejemplo una petición HTTPS. Y cuando decimos "de forma
...
### 4. El servidor de la aplicación recibe la petición de credenciales
...
seguridad proviene de una clave secreta que conocen únicamente los servidores de
Agora Voting y de Podemos. Con dicha clave y el mensaje, y mediante el
procedimiento de HMAC mediante hashes sha256, se genera un hash que autentica
la validez del mensaje ante Agora Voting.
...
de que haya dos votaciones simultánea.
...
de que el cálculo de expiración sea posible.
...
No obstante, recomendamos que directamente el servidor devuelva el enlace a la
cabina de votación, de manera que el cliente se limita a pedir dicho enlace (y
en el paso 5, abre dicho enlace en el navegador del usuario). Eso tiene la
ventaja de que si el enlace cambia, no hay que modificar el cliente, y además
en todo caso el cliente no debe preocuparse sobre la complejidad asociada a la
generación de dicho enlace.

Es por ello que explicamos en este paso cómo se generará dicho enlace. El enlace
será una URL que aun no funciona, pero que ya podemos decir que para la primera
votación tendrá el formato siguiente:

    http://agoravoting.github.io/agora-core-view/#/election/:id/vote/:hash/:message
...
Esta url tiene el siguiente formato:
    ...
    http://agoravoting.github.io/agora-core-view/#/test_hmac/:key/:hash/:message
...
siguiente es un ejemplo de una url generada correctamente:
    ...
    http://agoravoting.github.io/agora-core-view/#/test_hmac/whatever-password-123/ff66424d7d77607bbfe78209e407df6fff31abe214a1fe3b3a7dd82600ec0000/8dee0c135afeae29e208550e7258dab7b64fb008bc606fc326d41946ab8e773f:1:1411130040
